...
def get_update_status_and_data(first_start: bool, last_pal: bool):
    ...
    if not first_start:
        ...
        if rows:
            ...
            print("Сколько новых изделий на посту:", count_products, f"Номер последней палеты: {last_pal_num}")
            previous_datetime_row = max(rows, key=lambda x: x[3])
            previous_datetime = previous_datetime_row[3]
            last_pal_num = int(previous_datetime_row[0])
            if count_products:
        ...
        data = list(source_cursor.fetchmany(1)[0])
...
def convert_for_printer(row, marker_product) -> dict: # Функция отвечает за формирования переменных для маркиратора
    config.read('config.ini')
    tp = config.get('Settings', 'technological_post')
...
def processing_server_data(rows): # упорядовачивание данных, объект посредник конвертации данных
    server_data = []
    max_pos_x = max(int(rows[i][-1]) for i in range(len(rows)))
    for i in range(len(rows)):
        if len(rows) > 1:
            marker_product = 2 if int(rows[i][-1]) == max_pos_x else 1
        else:
            marker_product = 1
